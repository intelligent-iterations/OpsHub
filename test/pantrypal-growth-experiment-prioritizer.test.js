...
test('rankExperiments prioritizes higher PantryPal fit when readiness and score tie', () => {
  const ranked = rankExperiments([
    {
      name: 'Lower fit',
      impact: 0.8,
  ...
  assert.equal(ranked[0].name, 'Higher fit');
  assert.equal(ranked[1].name, 'Lower fit');
});
